...
public class Purchase extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("buy")!=null)
		{
			...
			pay.payByCredit(6, CardNo, todaysDate, 6, "Ek856", 2, 1500);
			int people=(Integer)request.getSession().getAttribute("noOfPassengers");
			
			System.out.println("passing reservationid "+reservationId+" and no of pass "+people);
			
			System.out.println("calling update");
			pay.updateStatus(reservationId,people);
		...
		else if(request.getParameter("cancel")!=null)
		{
			...
			HttpSession session=request.getSession();
			session.invalidate();
			response.sendRedirect("/AirlineMgmtSystemClient/index.jsp");
