...
public class EmployeeHelper {
	...
	public boolean updDeleteEmployee(PersonBean empObj, String emp, boolean isUpdate){
		...
		try{
			...
			PreparedStatement delStatement = con.prepareStatement("select * from Employee where Employee_id = ?");
			delStatement.setString(1, emp);
			ResultSet stat = delStatement.executeQuery();
			
			boolean status = false;
			if(getRowCount(stat) > 0){

			String temp = "update Employee set End_Date = ? where Employee_Id = ?";
			delStatement = con.prepareStatement(temp);
